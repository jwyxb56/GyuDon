...
namespace TJAPlayer3
{
    public static class DiscordRpc
    {
        ...
        public struct Et進行LoopentHandlers
        {
        ...
        public static extern void Initialize(string applicationId, ref Et進行LoopentHandlers handlers, bool autoRegister, string optionalSteamId);
        ...
        public static extern void UpdateHandlers(ref Et進行LoopentHandlers handlers);
    }
    ...
    public static class Discord
    {
        ...
        {
            var handlers = new DiscordRpc.Et進行LoopentHandlers();
        ...
        public static void UpdatePresence(string details, string state, long startTimeStamp = 0, long endTimeStamp = 0, string smallImageKey = null, string smallImageText = null)
        {
            ...
            presence.largeImageKey = StrToPtr("gyudon");
            presence.largeImageText = StrToPtr("GyuDon");
        ...
        private static IntPtr StrToPtr(string input)
        {
            ...
            var cont進行Loopbytecnt = Encoding.UTF8.GetByteCount(input);
            var buffer = Marshal.AllocHGlobal(cont進行Loopbytecnt + 1);
            for (int i = 0; i < cont進行Loopbytecnt + 1; i++)
            {
                Marshal.WriteByte(buffer, i, 0);
            }
            _buffers.Add(buffer);
            Marshal.Copy(Encoding.UTF8.GetBytes(input), 0, buffer, cont進行Loopbytecnt);
            return buffer;
        }
