...
namespace TJAPlayer3
{
    ...
    {
        #region [ コンストラクタ ]
        public CPrivateFont(FontFamily fontfamily, int pt, FontStyle style, int ed = 0, int tick = 0)
        {
            Initialize(null, fontfamily, pt, style, ed, tick);
        }
        public CPrivateFont(FontFamily fontfamily, int pt, int ed = 0, int tick = 0)
        {
            Initialize(null, fontfamily, pt, FontStyle.Regular, ed, tick);
        }
        public CPrivateFont(string fontpath, int pt, FontStyle style, int ed = 0, int tick = 0)
        {
            Initialize(fontpath, null, pt, style, ed, tick);
        }
        public CPrivateFont(string fontpath, int pt, int ed = 0, int tick = 0)
        {
            Initialize(fontpath, null, pt, FontStyle.Regular, ed, tick);
        }
        public CPrivateFont()
        {
        ...
        protected void Initialize(string fontpath, FontFamily fontfamily, int pt, FontStyle style, int ed, int tick)
		{
			if (ed == 0)
			{
				ed = (10 * pt / 30);
			...
			else
			{
				...
				{
					Trace.TraceWarning($"プライベートフォントの追加に失敗しました({fontpath})。代わりにMS UI Gothicの使用を試みます。");
			...
			// フォントファイルが見つからなかった場合 (MS UI Gothicを代わりに指定する)
			{
				try
				{
					_fontfamily = new FontFamily("MS UI Gothic");
					float emSize = pt * 96.0f / 72.0f;
				...
				{
					throw new FileNotFoundException($"プライベートフォントの追加に失敗し、MS UI Gothicでの代替処理にも失敗しました。({Path.GetFileName(fontpath)})", e);
				}
			}
		}
		...
		protected Bitmap DrawPrivateFont(string drawstr, DrawMode drawmode, Color fontColor, Color edgeColor, Color gradationTopColor, Color gradationBottomColor)
		{
			...
			int nEdgePt = (bEdge) ? (10 * _ed / 30) : 0; //SkinConfigにて設定可能に(rhimm)
		...
		protected Bitmap DrawPrivateFont_V(string drawstr, Color fontColor, Color edgeColor, bool bVertical)
		{
			...
			using (Graphics Gcambus = Graphics.FromImage(bmpCambus))
			{
				...
				for (int i = 0; i < strName.Length; i++)
				{
					...
					float nEdgePt = (10f * _ed / 30); // SkinConfigにて設定可能に(rhimm)
