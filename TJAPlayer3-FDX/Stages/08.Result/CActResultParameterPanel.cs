...
namespace TJAPlayer3
{
	internal class CActResultParameterPanel : CActivity
	{
		...
		public override int On進行描画()
		{
			...
			if (this.ctFire == null) this.ctFire = new CCounter(0, 6, 50, null);
			this.ctFire?.t進行Loop();
			if (TJAPlayer3.Tx.Result_Panel != null )
			{
                TJAPlayer3.Tx.Result_Panel.t2D描画( TJAPlayer3.Skin.nResultPanelP1X, TJAPlayer3.Skin.nResultPanelP1Y );
			}
            if(TJAPlayer3.Tx.Result_Gauge_Base != null && TJAPlayer3.Tx.Result_Gauge != null )
            {
                double Rate = TJAPlayer3.stage結果.st演奏記録.Drums.fゲージ;
				TJAPlayer3.Tx.Result_Gauge.t2D描画(TJAPlayer3.Skin.nResultGaugeBaseP1X, TJAPlayer3.Skin.nResultGaugeBaseP1Y, new Rectangle( 0, 0, (int)((TJAPlayer3.Tx.Result_Gauge.szテクスチャサイズ.Width / 50.0) * (Rate / 2)), 40 ) );
            ...
            if(TJAPlayer3.Tx.Gauge_Soul != null )
            {
				...
				{
					TJAPlayer3.Tx.Gauge_Soul_Fire.vc拡大縮小倍率.X = TJAPlayer3.Tx.Gauge_Soul_Fire.vc拡大縮小倍率.Y = 0.8f;
					TJAPlayer3.Tx.Gauge_Soul_Fire.t2D描画(490, 74, new Rectangle(230 * ctFire.n現在の値, 0, 230, 230));
					TJAPlayer3.Tx.Gauge_Soul_Fire.vc拡大縮小倍率.X = TJAPlayer3.Tx.Gauge_Soul_Fire.vc拡大縮小倍率.Y = 1f;
				}
				TJAPlayer3.Tx.Gauge_Soul.vc拡大縮小倍率.X = TJAPlayer3.Tx.Gauge_Soul.vc拡大縮小倍率.Y = 0.8f;
				TJAPlayer3.Tx.Gauge_Soul.t2D描画(547, 131, new Rectangle( 0, 0, 80, 80 ) );
				TJAPlayer3.Tx.Gauge_Soul.vc拡大縮小倍率.X = TJAPlayer3.Tx.Gauge_Soul.vc拡大縮小倍率.Y = 1f;
            ...
            if(TJAPlayer3.stage選曲.n確定された曲の難易度 == (int)Difficulty.Dan)
            {
                ...
                switch (TJAPlayer3.stage演奏ドラム画面.actDan.GetExamStatus(TJAPlayer3.stage結果.st演奏記録.Drums.Dan_C))
                {
                    case Exam.Status.Failure:
                        TJAPlayer3.Tx.Result_Dan?.t2D描画(TJAPlayer3.Skin.Result_Dan_XY[0], TJAPlayer3.Skin.Result_Dan_XY[1], new Rectangle(0, 0, TJAPlayer3.Skin.Result_Dan[0], TJAPlayer3.Skin.Result_Dan[1]));
                        break;
                    case Exam.Status.Success:
                        TJAPlayer3.Tx.Result_Dan?.t2D描画(TJAPlayer3.Skin.Result_Dan_XY[0], TJAPlayer3.Skin.Result_Dan_XY[1], new Rectangle(TJAPlayer3.Skin.Result_Dan[0], 0, TJAPlayer3.Skin.Result_Dan[0], TJAPlayer3.Skin.Result_Dan[1]));
                        break;
                    case Exam.Status.Better_Success:
                        TJAPlayer3.Tx.Result_Dan?.t2D描画(TJAPlayer3.Skin.Result_Dan_XY[0], TJAPlayer3.Skin.Result_Dan_XY[1], new Rectangle(TJAPlayer3.Skin.Result_Dan[0] * 2, 0, TJAPlayer3.Skin.Result_Dan[0], TJAPlayer3.Skin.Result_Dan[1]));
                        break;
                    default:
                        break;
                }
                // Dan_Plate
                Dan_Plate?.t2D中心基準描画(TJAPlayer3.Skin.Result_Dan_Plate_XY[0], TJAPlayer3.Skin.Result_Dan_Plate_XY[1]);
            }
            #endregion
            ...
            foreach (char ch in str)
            {
                for (int i = 0; i < this.stScoreFont.Length; i++)
                {
                    if (this.stScoreFont[i].ch == ch)
                    {
                        ...
                        {
                            TJAPlayer3.Tx.Result_Score_Number.t2D描画(x, y, rectangle);
                        }
                        break;
                    }
                }
