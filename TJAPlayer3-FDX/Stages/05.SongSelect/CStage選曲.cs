    ...
    internal class CStage選曲 : CStage
    {
		...
		public override void On活性化()
		{
			...
			try
			{
				...
				this.ctキー入力制限 = new CCounter(0, 100, 1, null);
		...
		public override int On進行描画()
		{
                ...
                if( this.r現在選択中の曲 != null )
                {
                    ...
                    if (TJAPlayer3.Tx.SongSelect_GenreBack[nGenreBack] != null )
                    {
                        for( int i = 0 ; i <(1280 / TJAPlayer3.Tx.SongSelect_Background.szテクスチャサイズ.Width) + 2; i++ )
                            if (TJAPlayer3.Tx.SongSelect_GenreBack[nGenreBack] != null )
                                    TJAPlayer3.Tx.SongSelect_GenreBack[nGenreBack].t2D描画(-ct背景スクロール用タイマー.n現在の値 + TJAPlayer3.Tx.SongSelect_Background.szテクスチャサイズ.Width * i , 0);
                    }
                }
			...
			//	this.bIsEnumeratingSongs = !this.actPreimageパネル.bIsPlayingPremovie;				// #27060 2011.3.2 yyagi: #PREMOVIE再生中は曲検索を中断する
				...
				if( this.ct登場時アニメ用共通.b進行中 )
				{
					...
					y = ( (int) (TJAPlayer3.Tx.SongSelect_Header.szテクスチャサイズ.Height * dbY表示割合 ) ) - TJAPlayer3.Tx.SongSelect_Header.szテクスチャサイズ.Height;
				}
				...
				if( TJAPlayer3.Tx.SongSelect_Footer != null )
                    TJAPlayer3.Tx.SongSelect_Footer.t2D描画( 0, 720 - TJAPlayer3.Tx.SongSelect_Footer.szテクスチャサイズ.Height );
                ...
                for (int i = 0; i < TJAPlayer3.ConfigIni.nPlayerCount; i++)
                {
                    ...
                    {
                        TJAPlayer3.Tx.NamePlate[i].t2D描画(TJAPlayer3.Skin.SongSelect_NamePlate_X[i], TJAPlayer3.Skin.SongSelect_NamePlate_Y[i]);
                    }
                }
                #endregion
                ...
                if (TJAPlayer3.Tx.SongSelect_Auto != null)
                {
                    ...
                    {
                        TJAPlayer3.Tx.SongSelect_Auto.t2D描画(TJAPlayer3.Skin.SongSelect_Auto_X[0], TJAPlayer3.Skin.SongSelect_Auto_Y[0]);
                    }
                    if (TJAPlayer3.ConfigIni.nPlayerCount > 1 && TJAPlayer3.ConfigIni.b太鼓パートAutoPlay2P)
                    {
                        TJAPlayer3.Tx.SongSelect_Auto.t2D描画(TJAPlayer3.Skin.SongSelect_Auto_X[1], TJAPlayer3.Skin.SongSelect_Auto_Y[1]);
                    }
                }
                ...
                TJAPlayer3.Tx.SongSelect_Difficulty.t2D描画( 980, 30, new Rectangle( 0, 70 * this.n現在選択中の曲の難易度, 260, 70 ) );
...
//Debug.WriteLine( "パンくず=" + this.r現在選択中の曲.strBreadcrumbs );
					...
					&& TJAPlayer3.act現在入力を占有中のプラグイン == null && this.ctキー入力制限.n現在の値 >= this.ctキー入力制限.n終了値)
				{
					...
					if ( !this.actSortSongs.bIsActivePopupMenu && !this.actQuickConfig.bIsActivePopupMenu /*&&  !this.act難易度選択画面.bIsDifficltSelect */ )
					{
						...
						#endregion
						#region [ F2 簡易オプション ]
						if ( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_F2 ) )
						{
						...
						if ( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_F3 ) )
						{
							TJAPlayer3.Skin.sound変更音.t再生する();
                        ...
                        if (TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_F4))
                        {
                        ...
                        #endregion
                        #region [ F5 スーパーハード ]
                        if ( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_F5 ) )
						{
							TJAPlayer3.Skin.sound変更音.t再生する();
						...
						#endregion
						#region [ F6 SCROLL ]
						if ( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_F6 ) )
						{
							TJAPlayer3.Skin.sound変更音.t再生する();
						...
						{
                            #region [ Decide ]
                            if (TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_F) || TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_J) || TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_RETURN))
                            {
                                if (this.act曲リスト.r現在選択中の曲 != null)
                                {
                            ...
                            #endregion
                            #region [ Up ]
                            this.ctキー反復用.Up.tキー反復( TJAPlayer3.Input管理.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_LEFT ), new CCounter.DGキー処理( this.tカーソルを上へ移動する ) );
							//this.ctキー反復用.Up.tキー反復( CDTXMania.Input管理.Keyboard.bキーが押されている( (int) DxLibDLL.DX.KEY_INPUT_UpArrow ) || CDTXMania.Input管理.Keyboard.bキーが押されている( (int) DxLibDLL.DX.KEY_INPUT_LeftArrow ), new CCounter.DGキー処理( this.tカーソルを上へ移動する ) );
							if (TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_D))
							{
								this.actPresound.sound?.t解放する();
								this.actPresound.sound = null;
								this.tカーソルを上へ移動する();
								GC.Collect();
							}
							#endregion
							#region [ Down ]
							this.ctキー反復用.Down.tキー反復( TJAPlayer3.Input管理.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_RIGHT ), new CCounter.DGキー処理( this.tカーソルを下へ移動する ) );
							//this.ctキー反復用.Down.tキー反復( CDTXMania.Input管理.Keyboard.bキーが押されている( (int) DxLibDLL.DX.KEY_INPUT_DOWN ) || CDTXMania.Input管理.Keyboard.bキーが押されている( (int) DxLibDLL.DX.KEY_INPUT_RightArrow ), new CCounter.DGキー処理( this.tカーソルを下へ移動する ) );
							if (TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_K))
							{
								this.actPresound.sound?.t解放する();
								this.actPresound.sound = null;
								this.tカーソルを下へ移動する();
							...
							#endregion
							#region [ Upstairs ]
							if (TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_ESCAPE))
							{
								this.actPresound.tサウンド停止();
								TJAPlayer3.Skin.sound取消音.t再生する();
							...
							*/
							...
							#region [ 上: 難易度変更(上) ]
							if( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_UP ) )
							{
							...
							#endregion
							#region [ 下: 難易度変更(下) ]
							if( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_DOWN ) )
							{
								//CommandHistory.Add( E楽器パート.DRUMS, EパッドFlag.HH );
								//EパッドFlag[] comChangeDifficulty = new EパッドFlag[] { EパッドFlag.HH, EパッドFlag.HH };
								//if ( CommandHistory.CheckCommand( comChangeDifficulty, E楽器パート.DRUMS ) )
  ...
  //      private CTexture tx下部テキスト;
        private CCounter ctDiffSelect移動待ち, ctキー入力制限;
...
//Debug.WriteLine("NOT start checking...stciCount=" + stciCount + ", targetCount=" + targetCount);
				...
				long curTime = DxLibDLL.DX.GetNowCount();
